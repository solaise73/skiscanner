...
var cache         = require('Simple-Cache').SimpleCache("tmp/websites/alpinresorts", console.log);
// var referenceData = require("./data/reference.json");
...
function setUpCountry(filterOn) {
  ...
  var url = 'https://www.alpinresorts.com/en/ski-rental/'+ theirCountryId;
  var model = {
    'resort': {
        ...
        if (path[3] && path[4] && path[5]){
          ...
          var params = {
            ...
            them: {
              ...
              'region_id': regionId,
              'supplier': 'alpinresorts'
            }
          }
          console.log('[alpinresorts] setup', resortId)
          promises.push(common.geoCodeResort(params))
        }
      })

      q.all(promises).then(function(){
        defer.resolve('[alpinresorts] All resorts mapped')
      })
  });
...
function fetchResort(paramsIn) {
  ...
  .then(function(params){
    var uri = 'https://www.alpinresorts.com/en/ski-rental/'+ params.them.country_id +'/'+ params.them.region_id +'/'+ params.them.resort_id;
    cache.get(uri, function(callback){
      shopsRequest = {
        uri: uri,
        headers: {}
      }
      console.log('[alpinresorts] fetchResort from ', shopsRequest.uri)
      ...
      shops.forEach(function(shop){
        if (shop.inTown) {
          ...
          common.findShop(p1).then(function(shop){
            ...
            console.log('[alpinresorts] fetchResort scrape shop', p2.us.shop_id, p2.them.shop_id)
          ...
          console.log('[alpinresorts] ignoring shop', shop.name, 'as not in resort', params.us.resort_id)
        }
      });
    })
  })
...
function processShop(params){
  ...
  console.log('[alpinresorts] processShop', dataUrl)
  params.them.products.forEach(function(product){
    if (['86','90','95'].indexOf(product.definition_id)<0){ // exclude womens for now
      ...
      delete p.category;
      delete p.level;
      console.log('[alpinresorts] saving a product', categoryId, levelId, params.us.shop_id, product.name)
      ...
      firebase.database().ref(dataUrl).child('shops').child(params.us.shop_id).child('suppliers/alpinresorts').child(categoryId).child(levelId).set(p);
      firebase.database().ref(dataUrl).child('shops').child(params.us.shop_id).child('suppliers/alpinresorts/best_discount').set(p.discount);
    } else {
...
function scrapeShopPrices(params) {
  var defer = q.defer();
  var uri = 'https://www.alpinresorts.com/en/service/static/ski-rental/shops/'+ params.them.shop_id +'/products?age_category=1&currency=EUR&duration=6';
  ...
  cache.get(uri, function(callback){
    ...
    console.log('[alpinresorts] scrapeShopPrices resolved', params.them.shop_id)
    defer.resolve(data)
  })
  return defer.promise;
}
...
function scrapeShopDiscounts(params) {
  var defer = q.defer();
  var uri = 'https://www.alpinresorts.com/en/service/ski-rental/shops/'+ params.them.shop_id +'/priceinfo';
  cache.get(uri, function(callback){
    var step1Request = {
    ...
    console.log('[alpinresorts] scrapeShopDiscounts resolved', params.them.shop_id)
...
function getProductDetails(productIn, discountData, duration){
  ...
  var product = {
    ...
    console.log('[alpinresorts] Got product', product.name, product.category, product.level, productIn.qualityCategoryId);
  return product;
}
...
function extractData(err, res, body){
  ...
  if (typeof body == 'string') {
    ...
    try {
      ...
      console.log('[alpinresorts] extractData VALID JSON');
    } catch (e) {
      console.log('[alpinresorts] extractData NOT JSON');
      data = body;
    }
  } else {
    data = body;
...
function mapParams(paramsIn) {
  ...
  var defer = q.defer();
  var key = 'resorts/'+ params.resort_id +'/alias/alpinresorts';
  var paramsOut = {
    us: params,
    them: {}
  };
  cache.get(key, function(callback){
    ...
    .then(function(resortSnap){
      if (resortSnap.exists()) {
        ...
        console.log('[alpinresorts] Unknown resort_id passed in', key);
      }
    })
  }).fulfilled(function(data) {
    paramsOut.them = data;
    paramsOut.them.supplier =  'alpinresorts';
