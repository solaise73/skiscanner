...
function getResortsAndShops() {
  Object.keys(referenceData.country).forEach(function(country){
    ...
    if (supplierCountryId==3){ /// TEMP
      ...
      console.log('snowrental:: Getting resort for country', country, supplierCountryId)
      resortRequest = {
      ...
      request(resortRequest, function(err, res, body){
        ...
        .then(function(data){
          data.response.list.forEach(function(resort){
            ...
            console.log('snowrental:: Getting shops for', resort.value, resort.key);
            firebase.database().ref().child('resorts/'+resortId+'/').child('alias/snowrental').update({'country_id': supplierCountryId, 'resort_id': supplierResortId});
    ...
    request(shopsRequest, function(err, res, body){
      ...
      .then(function(data){
        data.response.list.forEach(function(shop){
          ...
          console.log('snowrental::scrape shop', params.us.shop_id, params.them.shop_id)
          firebase.database().ref().child('shops').child(params.us.shop_id).update({
            name: shop.value,
            resort_id: params.us.resort_id,
          })
          firebase.database().ref().child('shops/suppliers/snowrental').set({
